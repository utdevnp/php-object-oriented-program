	...
	<thead>
		...
		<tr>
            ...
            <th> View Content </th>
			 <th>Status </th>
			<th colspan="4">
				<a title="Click Here To Add New Group" href="index.php?folder=group&file=add_group.php"> Add New Group</a>
			</th>
		</tr>
	</thead>
	...
	<?php
		...
		?>
			...
			<tr border="1">
				...
				<td><?php echo ($row['parent_id']==0)? "Self" : $Group->getNameById($row['parent_id']); ?></td>
				<td> <?php echo ($row['order_by']=='ASC') ? 'Ascending' : 'Descending'; ?> </td>
				<td> <?php echo $row['howmany'];?>  </td>
				<td> <?php echo $row['added_date'];?>  </td>
				<td> <a href="index.php?folder=content&file=list_content.php&group_id=<?php echo $row['id'];?>"><strong>Contents</strong></a></td>							
				
				<td class="<?php if($row['is_active']=='Y'){echo 'published';}else{echo 'unpublished';} ?>">
					  <?php if($row['is_active']=='Y') { ?>
						<a href="index.php?folder=group&file=unpublish_group.php&id=<?php echo $row['id'];?>"><strong><img src="graphics/activate.png" width="25" height="25" alt="Edit" title="Publish" /> </strong></a>
					<?php } else if($row['is_active']=='N') { ?>
					<a href="index.php?folder=group&file=publish_group.php&id=&id=<?php echo $row['id'];?>"><strong><img src="graphics/deactivate.png" width="25" height="25" alt="Edit" title="Unpublish" /></strong></a>
					<?php }?>
			  </td>
				
				<td> 
                	<a href="index.php?folder=group&file=view_group.php&id=<?php echo $row['id'];?>"> 
                    	<img src="graphics/view.png" width="25" height="25" alt="View" title="View" />
                    </a> 
                </td>
				<td> <a href="index.php?folder=group&file=edit_group.php&id=<?php echo $row['id'];?>"> 
