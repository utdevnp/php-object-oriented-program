...
<?php
	...
	$result = $Attachment->getById($_GET['id']);
		$count = $Conn->numrows($result);
		if($count==0)
		{
			header("Location:index.php?folder=attachment&file=list_attachment.php&content_id=$content_id&sub=$sub&group_id=$group_id&message=Record  Added Successfully");
		}
...
<?php
if(isset($_POST['edit_attachments_button']))
{
	if($_FILES['myfile']['name'] != "")
	{
		...
		//for upload file
			...
			$result = $Attachment-> edit($id,$content_id,$title,$is_active,$myfile);
	...
	else
	{
			...
			$counter = $Attachment->checkExistence($id);
		...
		if($result==1)
		{
			...
			header("Location:index.php?folder=attachment&file=list_attachment.php&content_id=$content_id&sub=$sub&group_id=$group_id&message= Update  Success!!!");
...
<div align="right">
		<h2 style="float:left; margin-left:5px; background:url(graphics/addnew/content.gif) no-repeat; text-indent:40px; Padding:5px;">Attachment >> Edit </h2>
...
<form method="post" action="" enctype="multipart/form-data">        
  ...
  <div class="form_item">
    <div class="">
      <input name="edit_attachments_button" class="submit_bitton" type="submit" value="Save & Update">
    </div>
  </div>
</form>
