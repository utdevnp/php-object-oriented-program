...
{
	$result = $Attachment->getByContentId($_GET['content_id']);	
}
else
{
...
?>
	...
	<div align="right">
		<h2 style="float:left; margin-left:5px; background:url(graphics/addnew/content.gif) no-repeat; text-indent:40px; Padding:5px;">Attachment Content</h2>
	...
	<thead>
		...
		<tr>
			...
			<th colspan="4">
                ...
                <a href="index.php?folder=attachment&file=add_attachment.php&content_id=<?php echo $_GET['content_id'];?>&group_id=<?php echo $_GET['group_id'];?>&sub=<?php echo $sub; ?>"> 
                	ADD NEW 
                </a>
                <?php }else{?>
                	&darr;
		...
		?>		
			<tr>
                ...
                <td> 
					<a href="<?php echo ATTACH_FILE_DIR . $row['id'].".".$row['myfile'];?>" target="_blank">
                    	Download
                    </a>
				</td>
                
				<td> <a href="index.php?folder=attachment&file=view_attachment.php&content_id=<?php echo $_GET['content_id'];?>&group_id=<?php echo $_GET['group_id'] ?>&id=<?php echo $row['id'];?>&sub=<?php echo $sub; ?>"> 
                <img style="float:left;" src="graphics/view.png" width="25" height="25" alt="View" title="View" /><small class="edit">view</small> </a> </td>
				<td> <a href="index.php?folder=attachment&file=edit_attachment.php&content_id=<?php echo $_GET['content_id'];?>&group_id=<?php echo $_GET['group_id'] ?>&id=<?php echo $row['id'];?>&sub=<?php echo $sub; ?>"> 
                <img style="float:left;" src="graphics/edit.png" width="25" height="25" alt="Edit" title="Edit" /> <small class="edit">Edit</small>  </a> </td>
				<td> <a href="index.php?folder=attachment&file=delete_attachment.php&id=<?php echo $row['id'];?>&content_id=<?Php echo $_GET['content_id'];?>&group_id=<?php echo $_GET['group_id'];?>&sub=<?php echo $sub; ?>">
