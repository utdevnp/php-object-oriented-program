...
class Activity
{
	...
	Public function add($parent_id,$name,$is_active,$description)
	{
		...
		//prepare sql
		$sql = "INSERT INTO activity(parent_id,name,is_active,description) VALUES($parent_id,'$name','$is_active','$description')";
	...
	{
		global $Conn;
		$sql = "SELECT id, name FROM activity WHERE parent_id=0";
		$result = $Conn->execute($sql);	
		return $result;
	}	
