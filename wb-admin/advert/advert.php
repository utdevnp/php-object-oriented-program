...
class Advert
{
	...
	//Update for edit
	public function updateOld($id,$advertisement_id,$title,$url,$caption,$is_active)
	{
		global $Conn;
		$id=  $Conn->clean($id);
		$advertisement_id=  $Conn->clean($advertisement_id);
	...
	public function getById($id)
	{
		global $Conn;	
		$id=  $Conn->clean($id);
		$sql = "SELECT * FROM advert WHERE id=$id AND is_active='Y'";
		$result = $Conn->execute($sql);
		return $result;	
	}
	...
	public function getByAdvertisementId($advertisement_id)
	{
		global $Conn;	
		$advertisement_id=  $Conn->clean($advertisement_id);
		$sql = "SELECT * FROM advert WHERE advertisement_id=$advertisement_id";
		$result = $Conn->execute($sql);
		return $result;	
	}
	...
	public function getByAdvertIdWithLimit($advertisement_id,$limit)
	{
		global $Conn;
		$advertisement_id=  $Conn->clean($advertisement_id);
		$sql="SELECT * FROM advert WHERE advertisement_id=$advertisement_id AND is_active='Y'  ORDER BY id DESC LIMIT $limit";
		$result=$Conn->execute($sql);
		return $result;
	}	
	...
	function GetPhotosIdWithLimit($advertisement_id)
	{
		...
		$sql = "SELECT * FROM advert WHERE  advertisement_id =$advertisement_id AND is_active =  'Y' ORDER BY id $order_by LIMIT $howmany" ;
		$result=$Conn->execute($sql);
		return $result;
	}
	
	
	function GetGallerydWithLimit($advertisement_id)
	{
		global $Conn;
		global $Advertisement;
		$advertisement_id=  $Conn->clean($advertisement_id);
	...
	{
		global $Conn;
		$sql="SELECT COUNT(*) As counter FROM advert
		WHERE
			id=$id";
	...
	{
		global $Conn;	
			$sql="UPDATE advert SET ext='$ext' WHERE id=$id  ";
				//$result=mysql_query($sql, $Conn);
				$result=$Conn->execute($sql, $Conn);
					return $result;
	}
